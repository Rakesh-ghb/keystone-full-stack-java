...
public class JwtService {
    ...
    public boolean valid(String token, UserDetails user) {
        try { return username(token).equalsIgnoreCase(user.getUsername()) && !Jwts.parser().verifyWith(key).build().parseSignedClaims(token).getPayload().getExpiration().before(new Date()); }
